...
if(box){
    ...
    window.onscroll = function(){
        ...
        if(revise){
            ...
            revise.style = win_scrol>H?"position:absolute;top:"+(win_scrol-13)+"px":"";
        }
    }
        ...
        for (let i = 0; i < revises.length; i++) {
            revises[i].addEventListener('click',function () {
                if (this.parentNode.nextElementSibling == null || this.parentNode.nextElementSibling.classList.toString().indexOf("editor") < 0) {
                    ...
                    editor.customConfig.menus = ['bold','italic','underline','link','undo','redo'];
                    ...
                    let zls_nick_user = storage.getItem("zls_nick_user");
                    if(zls_nick_user){
                    ...
                    element.addEventListener('click',function (){
                        ...
                        storage.setItem("zls_nick_user",zls_nick_user);
            ...
            if (res.ret == 0) {
                $.ajax({
                    ...
                    success:function (data){
                        if(data)saveRevise(that,source,id,nickName,email,title,content,res.randstr,res.ticket);
                    }
                })
            }
        });
...
;(function () {
    ...
    function zh_tran(go,obj) {
        if (go) zh_choose = go;
        storage.setItem('zh_choose', zh_choose);
        if (go == 'n') {
    ...
    function zh_getLang() {
        if (storage.getItem('zh_choose')) {
            zh_choose = storage.getItem('zh_choose');
            return true;
        }
    ...
    function obtainParentNode(obj,cls) {
        if (obj.parentNode.classList.contains(cls)){
...
;(function () {
    ...
    function refresh_login() {
        ...
        let lgn_out = document.querySelector('.lgn-out');
        let lgn_in = document.querySelectorAll('.lgn-in');
        let lgn_up = document.querySelectorAll('.lgn-up');
        let lgn_button = document.querySelector('.lgn-button');
        let lgn_text = document.querySelector('.lgn-text');
        if(lgn){
            if(lgn_in){
                lgn_in.forEach(function (el) {
                    el.addEventListener('click',lgn_in_f);
                })
            }
            if(lgn_up){
                lgn_up.forEach(function (el) {
                    el.addEventListener('click',lgn_up_f);
                })
            }
            if(lgn_out)lgn_out.addEventListener('click',logout);
            if(cookie.getCookie('zls_auth')){
                let item = storage.getItem('zls_user');
                if(item){
                    lgn_text.innerHTML = "欢迎:"+item.username;
                    classie.remove(lgn_out,'is-hidden');
                    classie.add(lgn_button,'is-hidden');
                }else{
                    lgn_text.innerHTML = "";
                    classie.remove(lgn_button,'is-hidden');
                    classie.add(lgn_out,'is-hidden');
                    //进入这里等于出现BUG,存在token却无用户信息，则移除token,使其重新登陆
                    cookie.delCookie('zls_auth');
                }
            }else{
                lgn_text.innerHTML = "";
                classie.remove(lgn_button,'is-hidden');
    ...
    function init_login(){
        ...
        if(window.location.href.indexOf("?login")>=0){
            if (cookie.getCookie("zls_auth")=='')lgn_in_f();
        }
    ...
    function clear_error() {
        ...
        message_login[1].innerHTML = "&nbsp;";
    }
    function captchaLogin() {
        clear_error();
        let username = document.getElementById("in_username").value;
        let password = document.getElementById("in_password").value;
        if(username == null || username.trim() == ""){
            message_login[0].innerHTML = "请输入账户";
            return;
        }
        if(password == null || password.trim() == ""){
            message_login[0].innerHTML = "请输入密码";
            return;
        }
        let captcha1 = new TencentCaptcha('2002020665', function(res) {
            // res（用户主动关闭验证码）= {ret: 2, ticket: null}
            ...
            if(res.ret == 0){
                $.ajax({
                    ...
                    success:function (flag) {
                        if(flag){
                            login(username,password,res.randstr,res.ticket);
                        }else{
                            captchaLogin();
                        }
                    }
    ...
    function login(username,password,randStr,ticket) {
        ...
        $.ajax({
            ...
            success:function (res,xhr) {
                ...
                if(data.code == 1){
                    cookie.delCookie("zls_auth");
                    cookie.setCookie("zls_auth",xhr.getResponseHeader("Cookie"),1);
                    storage.setItem("zls_user",data.data);
                    let notification = new NotificationFx({
                        message : '登陆成功',
                        ttl:3000,
                    });
